import { FlatList, StyleSheet, Text, View, Image, RefreshControl, ScrollView, Pressable } from 'react-native'
  ...
  const ChatsPerson = () => (
    <>
      <ScrollView style={{}}>
        {
          temp.map(item => (
            <Pressable key={item.key} onPress={()=>navigation.navigate('PersonalChatScreen',{ChatTo:item})} style={{ backgroundColor: 'lightgray', padding: 6, borderBottomWidth: 1, borderRadius: 0, flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center' }}>
              <View style={{ flexDirection: 'row', alignItems: 'center' }}>
                ...
                <View>
                  <Text key={item.key} >{item.value[0].senderName}</Text>
